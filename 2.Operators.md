...
You are to implement a class for working with Complex numbers

Create a `Complex` struct (cause we prefer structs to classes when it comes down immutability) that supports all arithmetic operations on complex numbers. Namely, you class should have the following public inteface:

```c++
// An immutable class for working with complex numbers.
...
Your struct MUST be IMMUTABLE with the ONLY exception of that rule being the input operator (`std::istream& operator>>(std::istream& stream, const Complex& number)`). In other words, say that `p` is an instance of `Complex`. Then no method or field should be able to change the values of `p`'s real and imaginary parts besides the operators that reads from the console.
...
cout << one.GetReal() == 1; // true;
cout << i.GetImaginary() == 0; // true;
